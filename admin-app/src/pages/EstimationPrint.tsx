...
function MobilePDFView({ estimation }: { estimation: any }) {
    ...
    useEffect(() => {
        ...
        const generate = async () => {
            try {
                const blob = await pdf(<EstimationPDF estimation={estimation} />).toBlob()
                if (cancelled) return
    ...
    const handleDownload = () => {
        ...
        link.href = blobUrl
        link.download = `Estimation-${estimation.estimation_number}.pdf`
    ...
    const handleShare = async () => {
        ...
        try {
            ...
            const file = new File([blob], `Estimation-${estimation.estimation_number}.pdf`, { type: 'application/pdf' })
            if (navigator.share && navigator.canShare({ files: [file] })) {
                await navigator.share({ files: [file], title: `Estimation ${estimation.estimation_number}` })
            } else {
                handleDownload()
            }
        } catch {
...
export default function EstimationPrint() {
    const { id } = useParams()
    const { data: estimationResult, isLoading, error } = useEstimationForPrint(id as string)
    ...
    if (error) return <div className="p-8 text-center text-red-500">Error loading estimation: {error instanceof Error ? error.message : 'An unexpected error occurred'}</div>
    ...
    if (isMobile) {
        return (
            <>
                ...
                <MobilePDFView estimation={estimation} />
            </>
        )
    }
