...
-- =====================================================
-- ESTIMATIONS (Spare Parts Quotations)
-- =====================================================

-- ESTIMATIONS
CREATE TABLE estimations (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  estimation_number VARCHAR(20) UNIQUE,
  customer_id UUID REFERENCES customers(id),
  vehicle_id UUID REFERENCES vehicles(id),
  ...
  validity_period INTEGER DEFAULT 15,
  valid_until DATE,
...
CREATE INDEX idx_estimations_number ON estimations(estimation_number);
CREATE INDEX idx_estimations_customer ON estimations(customer_id);
...
-- Sequence for estimation numbers
CREATE SEQUENCE IF NOT EXISTS estimation_number_seq START WITH 1;

-- Auto-generate EST-XXXX estimation numbers
CREATE OR REPLACE FUNCTION generate_estimation_number()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.estimation_number IS NULL OR NEW.estimation_number = '' THEN
    NEW.estimation_number := 'EST-' || LPAD(nextval('estimation_number_seq')::text, 4, '0');
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER auto_estimation_number
  BEFORE INSERT ON estimations
  FOR EACH ROW EXECUTE FUNCTION generate_estimation_number();

-- Auto-compute valid_until from estimation_date + validity_period
CREATE OR REPLACE FUNCTION compute_valid_until()
RETURNS TRIGGER AS $$
BEGIN
  NEW.valid_until := NEW.estimation_date + (NEW.validity_period || ' days')::interval;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER auto_valid_until
  BEFORE INSERT OR UPDATE ON estimations
  FOR EACH ROW EXECUTE FUNCTION compute_valid_until();

-- Apply updated_at trigger
CREATE TRIGGER update_estimations_timestamp BEFORE UPDATE ON estimations FOR EACH ROW EXECUTE FUNCTION update_updated_at();
